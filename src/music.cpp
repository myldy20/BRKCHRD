...
ColourResult normal_colour(Quality q, Direction d) {
    ...
    switch (d) {
    case Direction::Up: return q == Quality::Major ? ColourResult{{0,3,7},"m","FLIP"} : ColourResult{{0,4,7},"","FLIP"};
    case Direction::UpRight: return q == Quality::Minor ? ColourResult{{0,3,7,10},"m7","MIN7"} : ColourResult{{0,4,7,10},"7","DOM7"};
    case Direction::Right: return q == Quality::Major ? ColourResult{{0,4,7,11},"maj7","MAJ7"} : ColourResult{{0,3,7,10},"m7","MIN7"};
    case Direction::DownRight: return q == Quality::Minor ? ColourResult{{0,3,7,10,14},"m9","MIN9"} : ColourResult{{0,4,7,14},"add9","ADD9"};
    case Direction::Down: return {{0,5,7},"sus4","SUS4"};
    case Direction::DownLeft: return q == Quality::Major ? ColourResult{{0,4,7,9},"6","SIXTH"} : ColourResult{{0,2,7},"sus2","SUS2"};
    case Direction::Left: return q == Quality::Major ? ColourResult{{0,3,7},"m","DARK"} : ColourResult{{0,3,6},"dim","DIM"};
    case Direction::UpLeft: return {{0,q == Quality::Minor ? 3 : 4,8},q == Quality::Minor ? "m+" : "aug","AUG"};
...
ColourResult extended_colour(Quality q, Direction d) {
    ...
    case Direction::Right: return {{0,4,7,9,14},"6/9","SIXNINE"};
    case Direction::DownRight: return {{0,3,7,10,14,17},"m11","MIN11"};
    case Direction::Down: return {{0,5,7,10},"7sus4","7SUS"};
    case Direction::DownLeft: return {{0,3,6,10},"m7b5","HALFDIM"};
    ...
    for (int inversion = 0; inversion < count; ++inversion) {
        ...
        for (int shift : {-12, 0, 12}) {
            ...
            for (const int note : shifted) {
                int distance = std::abs(note - previous.front());
                for (const int old : previous) distance = std::min(distance, std::abs(note - old));
...
std::string direction_label(int colour_palette, Direction direction) { return apply_colour(Quality::Major, colour_palette, direction).label; }
